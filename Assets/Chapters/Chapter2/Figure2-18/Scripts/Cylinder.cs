    ...
    public class Cylinder : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            for (var i = 0; i < seg; i++)
            {
                ...
                var x = radius * Mathf.Cos(theta);
                var y = radius * Mathf.Sin(theta);
                vertices.Add(new Vector3(x, y, bottom));    // Lower vertex of the side face
                vertices.Add(new Vector3(x, y, top));       // Upper vertex of the side face
            }
            ...
            for (var i = 0; i < seg; i++)
            {
                triangles.Add(margin + i * 2);                   // Upper face
                triangles.Add(margin + ((i + 1) % seg) * 2 + 1);
                triangles.Add(margin + i * 2 + 1);
                triangles.Add(margin + i * 2);                   // Lowwer face
                triangles.Add(margin + ((i + 1) % seg) * 2); 
                triangles.Add(margin + ((i + 1) % seg) * 2 + 1);
            }
