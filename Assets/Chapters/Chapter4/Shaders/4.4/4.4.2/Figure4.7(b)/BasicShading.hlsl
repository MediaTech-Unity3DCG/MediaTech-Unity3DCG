...
float3 uniformHemiCosine(float2 r) {
    float cosTht2 = r.x;
    float sinTht2 = max(1.0 - cosTht2, 0.0);
    float sinTht = sqrt(sinTht2);
    float cosTht = sqrt(cosTht2);
    float phi = 2.0 * UNITY_PI * r.y;
    float sinPhi; float cosPhi; sincos(phi, sinPhi, cosPhi);
    return float3(sinTht * cosPhi, sinTht * sinPhi, cosTht);
}
