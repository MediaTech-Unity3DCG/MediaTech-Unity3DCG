...
namespace Exercise5 {
    ...
    public class CameraMover : MonoBehaviour
    {
        ...
        private void CameraPositionKeyControl()
        {
            ...
            if (Keyboard.current.dKey.isPressed) { campos += _camTransform.right * Time.deltaTime * _positionStep; }
            if (Keyboard.current.aKey.isPressed) { campos -= _camTransform.right * Time.deltaTime * _positionStep; }
            if (Keyboard.current.eKey.isPressed) { campos += _camTransform.up * Time.deltaTime * _positionStep; }
            if (Keyboard.current.qKey.isPressed) { campos -= _camTransform.up * Time.deltaTime * _positionStep; }
            if (Keyboard.current.wKey.isPressed) { campos += _camTransform.forward * Time.deltaTime * _positionStep; }
            if (Keyboard.current.sKey.isPressed) { campos -= _camTransform.forward * Time.deltaTime * _positionStep; }
